                ...
                <div className="space-y-3">
                    {maintenanceAlerts.length > 0 ? (
                        maintenanceAlerts.map((alert, idx) => (
                            <div key={idx} className="p-3 bg-rose-50 dark:bg-rose-500/10 rounded-xl border border-rose-100 dark:border-rose-500/20 group cursor-pointer hover:bg-rose-100 dark:hover:bg-rose-500/20 transition">
                                <p className="text-xs font-bold text-rose-800 dark:text-rose-300">{alert.name}</p>
                                <p className="text-[11px] text-rose-600 dark:text-rose-400 mt-1">{alert.condition}</p>
                            </div>
                        ))
                    ) : (
                        ...
                        return (
                            <div key={idx} className="group">
                                <div className="flex justify-between text-[13px] mb-1.5">
                                    <span className="text-slate-700 dark:text-slate-300 group-hover:text-cyan-600 transition-colors">{item.title}</span>
                                    <span className="text-slate-400 text-[11px]">{item.value} / {item.target}</span>
                                </div>
                                <div className="w-full bg-slate-100 dark:bg-slate-700 rounded-full h-2 overflow-hidden">
                                    <div 
                                        className={`h-full rounded-full transition-all duration-1000 ${percentage >= 90 ? 'bg-gradient-to-r from-emerald-400 to-emerald-600' : 'bg-gradient-to-r from-cyan-400 to-blue-500'}`} 
