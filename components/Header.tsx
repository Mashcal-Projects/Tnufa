...
import { Search, Bell, User, LayoutGrid, ShieldCheck, Database, Zap, LogOut } from 'lucide-react';
  ...
  return (
    <header className={`h-16 border-b backdrop-blur-md sticky top-0 z-40 px-6 flex items-center justify-between transition-colors duration-300
      ...
      <div className="flex items-center gap-4 w-full md:w-auto justify-end">
        ...
        <div className="flex items-center gap-3 cursor-pointer group relative" ref={menuRef}>
          <div
          ...
          {/* Logout Dropdown */}
          {isMenuOpen && (
            <div className={`absolute left-0 top-full mt-2 w-48 rounded-2xl shadow-2xl border p-2 animate-in fade-in slide-in-from-top-2 duration-200
              ${theme === 'dark' ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-200'}`}>
              <div className="px-3 py-2 border-b border-slate-100 dark:border-slate-800 mb-1">
                <p className="text-[10px] font-bold text-slate-400 mb-0.5">מחובר כ:</p>
                <p className={`text-[11px] truncate font-medium ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>{user?.email}</p>
              </div>
              <button
                onClick={() => logout()}
